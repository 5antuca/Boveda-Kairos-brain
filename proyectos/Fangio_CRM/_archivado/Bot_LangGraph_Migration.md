...
# Migración del cerebro FangioCRM: n8n LangChain → LangGraph (Python)

## Contexto

El doc [[Arquitectura_SaaS_Multitenant]] diseñó la arquitectura SaaS de FangioCRM con **n8n LangChain como AI Agent**: cuando llega un mensaje WhatsApp por Evolution → fangiocrm-n8n-master fetch a `fangiocrm.com/api/agent/context?instance={X}` → AI Agent (LangChain en n8n) contesta.
...
Cada tenant de FangioCRM tiene **su propio agente LangGraph** parametrizado dinámicamente desde `fangiocrm.com`. La concesionaria Trébol Automotores pasa a ser un tenant más (instance `el-trebol`) — los `configs/trebol.yaml` quedan como fallback.

El cliente final del SaaS personaliza el bot desde la UI de FangioCRM:
...
- Bot deja de leer `configs/{client_id}.yaml` y empieza a fetchear `https://fangiocrm.com/api/agent/context?instance={X}` en runtime.
...
- Bypass total de Chatwoot para FangioCRM tenants (Chatwoot solo lo usa Trebol legacy).
...
### Fase 5 — UI de personalización (FangioCRM frontend, fuera de este repo)
- Sección "Configurar Bot" en el dashboard del tenant en `fangiocrm.com`.
...
1. **¿`/api/agent/context` ya está implementado?** El doc lo describe pero hay que verificar contra el código de FangioCRM (`/root/apps/FangioCRM/backend/`).
2. **URL real del endpoint** (prod `fangiocrm.com` o staging?).
...
- Trebol prod sigue apagado desde 2026-04-26 (cliente de baja). Si vuelven, reactivamos como tenant FangioCRM, no como stack standalone.

## Links

- [[Arquitectura_SaaS_Multitenant]] — diseño SaaS original
- [[Fangio_CRM]] — visión general
