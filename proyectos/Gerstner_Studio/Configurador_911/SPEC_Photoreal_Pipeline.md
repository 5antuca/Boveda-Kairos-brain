...
- **BUG RAÍZ #2: materiales BSDF_DIFFUSE → salen BLANCO PURO (1,1,1) en glTF.** El exporter glTF solo lee el color del Principled BSDF, NO del Diffuse → todos los materiales Diffuse del pack (`Plastic_int_matt`=PISO interno bajo butacas/pedaleras, `Plastic_gauge_bck`, `Plastic_button_matt`, `Radio_screen`, `Headlining`, `Carpet`, etc.) exportan con baseColor blanco. Por eso el **piso interno se veía blanco** y nada lo arreglaba (yo seteaba roughness pero no color). **Fix en `Car.tsx` COLOR_MATS**: forzar el color oscuro real a cada uno (`Plastic_int_matt #161616`, etc.). **Para Fase 2: convertir los Diffuse→Principled en Blender** así el color exporta bien de origen.
- **Llantas: revertidas a plata por defecto** (el anodizado negro no gustó). `Fuel_oil_caps` → metal `#cccccc`.
- Pendiente: bugs visuales (z-fighting) en el **acrílico verde del spoiler** y en **ópticas/faros** (capas transparentes solapadas del pack); se probó draco position 16 (de 14) para mitigar. Necesita trabajo de geometría en Blender (Fase 2).
