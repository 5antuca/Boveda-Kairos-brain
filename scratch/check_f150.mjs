...
try {
  const folder = "Ford SVT F150 Lightning";
  console.log(`Checking folder: ${folder}`);
  const resources = await searchCloudinaryResources(`asset_folder="${folder}" AND resource_type:(image OR video)`);
  console.log("Found resources:", resources.map(r => r.public_id));
} catch (err) {
  console.error(err);
}
